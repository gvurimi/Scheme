...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError
        def bind (formals, vals):
            formal = formals.first
            value = vals.first
            child.define(formal, value)
            formals = formals.second
            vals = vals.second
            if formals != nil:
                return bind(formals, vals)
        # END PROBLEM 11
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 13
    "*** REPLACE THIS LINE ***"
    def and_eval(expressions, env):
        if expressions.second == nil:
            return scheme_eval(expressions.first, env)
        elif scheme_truep(scheme_eval(expressions.first, env)) == False:
            return False
        else:
            return and_eval(expressions.second, env)

    if expressions == nil:
        return True
    else:
        return and_eval(expressions, env)
    # END PROBLEM 13
...
def do_or_form(expressions, env):
    ...
    def and_eval(expressions, env):
        if expressions.second == nil:
            return scheme_eval(expressions.first, env)
        elif scheme_truep(scheme_eval(expressions.first, env)) == True:
            return scheme_eval(expressions.first, env)
        else:
            return and_eval(expressions.second, env)
...
def make_let_frame(bindings, env):
    ...
    x = bindings 
    while x != nil:
        check_form(x.first, 0, 2)
        x = x.second
    
    # BEGIN PROBLEM 15
    "*** REPLACE THIS LINE ***"
    def make_formals(bindings, env):
        if bindings != nil:
            if bindings.second == nil:
                formals = Pair(bindings.first.first, nil)
                return formals
            else:
                formals = Pair(bindings.first.first, make_formals(bindings.second, env))
        return formals
            
            
    def make_values(bindings, env):
        if bindings != nil:
            if bindings.second == nil:
                values = Pair(scheme_eval(bindings.first.second.first, env), nil)
                return values
            else:
                values = Pair(scheme_eval(bindings.first.second.first, env), make_values(bindings.second, env))
            
        return values
    
   
    variables = make_formals(bindings, env)
    check_formals(variables)
    values = make_values(bindings, env)
    return env.make_child_frame(variables, values)
...
class MuProcedure(UserDefinedProcedure):
    ...
    LambdaProcedure.make_call_frame
    # END PROBLEM 16
